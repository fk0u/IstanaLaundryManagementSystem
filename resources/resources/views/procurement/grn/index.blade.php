<x-app-layout>
    <div x-data="grnApp(@js($activePos))" class="flex flex-col gap-6">
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-slate-850">
                        @forelse($grns as $grn)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                                ...
                                <td class="py-4 px-4">
                                    <div class="font-mono font-bold text-primary dark:text-orange-400">
                                        {{ $grn->purchaseOrder?->po_number ?? '-' }}
                                    </div>
                                    <div class="text-[10px] text-slate-500 font-semibold">
                                        {{ $grn->purchaseOrder?->supplier?->name ?? 'Supplier' }}
                                    </div>
                                </td>
                                ...
                                <td class="py-4 px-4 text-center font-bold text-slate-700 dark:text-slate-300">
                                    <span class="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 font-mono">
                                        {{ $grn->items->count() }} Jenis
                                    </span>
                                </td>
                                <td class="py-4 px-4">
                                ...
                                <td class="py-4 px-4 text-right">
                                    <div class="flex justify-end items-center gap-2">
                                        <button type="button" @click="viewDetail(@js($grn))" 
                                        ...
                                        @if ($grn->status === 'draft')
                                            <form action="{{ route('procurement.grns.confirm', $grn->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mengonfirmasi penerimaan barang ini? Tindakan ini akan menambah stok persediaan FIFO dan memposting jurnal keuangan secara otomatis.')">
                                                @csrf
                                                <button type="submit" class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-2xs font-extrabold cursor-pointer transition-all shadow-xs">
                                                    Konfirmasi
                                                </button>
                                            </form>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="py-12 text-center text-slate-400">Belum ada penerimaan barang (GRN) tercatat.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <!-- Detail GRN Modal -->
        <template x-if="activeDetailGrn">
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" x-cloak @keydown.escape.window="activeDetailGrn = null">
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-3xl w-full p-6 shadow-2xl space-y-4 max-h-[90vh] flex flex-col overflow-hidden">
                    <div class="flex justify-between items-center pb-4 border-b border-slate-100 dark:border-slate-800">
                        <div>
                            <div class="flex items-center gap-2">
                                <h3 class="text-base font-black text-slate-800 dark:text-slate-200" x-text="'Detail GRN ' + activeDetailGrn.grn_number"></h3>
                                <template x-if="activeDetailGrn.status === 'confirmed'">
                                    <span class="px-2.5 py-0.5 rounded-full text-2xs font-extrabold bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-400">● Terkonfirmasi</span>
                                </template>
                                <template x-if="activeDetailGrn.status !== 'confirmed'">
                                    <span class="px-2.5 py-0.5 rounded-full text-2xs font-extrabold bg-slate-100 text-slate-600">Draft</span>
                                </template>
                            </div>
                            <p class="text-2xs text-slate-400 mt-0.5" x-text="'Ref PO: ' + (activeDetailGrn.purchase_order ? activeDetailGrn.purchase_order.po_number : '-') + ' | Supplier: ' + (activeDetailGrn.purchase_order && activeDetailGrn.purchase_order.supplier ? activeDetailGrn.purchase_order.supplier.name : '-')"></p>
                        </div>
                        <button type="button" @click="activeDetailGrn = null" class="text-slate-400 hover:text-slate-600 cursor-pointer">
                    ...
                    <div class="flex-1 overflow-y-auto space-y-4 pr-1">
                        <!-- Info Grid -->
                        <div class="grid grid-cols-3 gap-3 p-3 bg-slate-50 dark:bg-slate-850 rounded-2xl border border-slate-100 dark:border-slate-800 text-xs">
                            <div>
                                <span class="text-[10px] font-bold text-slate-400 uppercase block">Tanggal Terima</span>
                                <span class="font-semibold text-slate-800 dark:text-slate-200" x-text="activeDetailGrn.received_date ? new Date(activeDetailGrn.received_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }) : '-'"></span>
                            </div>
                            <div>
                                <span class="text-[10px] font-bold text-slate-400 uppercase block">Penerima Gudang</span>
                                <span class="font-semibold text-slate-800 dark:text-slate-200" x-text="activeDetailGrn.received_by ? activeDetailGrn.received_by.name : '-'"></span>
                            </div>
                            <div>
                                <span class="text-[10px] font-bold text-slate-400 uppercase block">Total Nilai Finansial</span>
                                <span class="font-black text-emerald-600 dark:text-emerald-400 font-mono text-sm" x-text="'Rp ' + formatNumber(calculateGrnTotal(activeDetailGrn))"></span>
                            </div>
                        </div>

                        <template x-if="activeDetailGrn.notes">
                            <div class="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 text-xs text-amber-800 dark:text-amber-300">
                                <span class="font-bold block text-[10px] uppercase">Catatan Penerimaan:</span>
                                <span x-text="activeDetailGrn.notes"></span>
                            </div>
                        </template>

                        <!-- Items Table -->
                        <div class="space-y-2">
                            <span class="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider">Rincian Barang & Batch FIFO</span>
                                ...
                                <table class="w-full text-left text-xs">
                                    <thead class="bg-slate-100 dark:bg-slate-800/80 text-slate-500 font-bold uppercase text-[10px]">
                                        <tr>
                                            ...
                                            <th class="py-2.5 px-3 text-center">Batch ID</th>
                                    ...
                                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                        <template x-for="item in activeDetailGrn.items" :key="item.id">
                                            <tr>
                                                <td class="py-2.5 px-3 font-semibold text-slate-800 dark:text-slate-200" x-text="item.item ? item.item.name : 'Barang #' + item.item_id"></td>
                                                <td class="py-2.5 px-3 text-center font-mono text-[10px] text-slate-500" x-text="item.batch_number || '-'"></td>
                                                ...
                                                <td class="py-2.5 px-3 text-right font-mono" x-text="formatNumber(item.unit_cost)"></td>
                                                <td class="py-2.5 px-3 text-right font-mono font-bold text-slate-900 dark:text-slate-100" x-text="formatNumber(item.quantity * item.unit_cost)"></td>
                                            </tr>
                                        </template>
                                    </tbody>
                                </table>
                        ...
                        <!-- Journal Sync Info Banner -->
                        <div class="p-3 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-xs font-semibold flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <span class="material-symbols-outlined text-lg">sync</span>
                                <span>Jurnal Akuntansi: Dr. Persediaan BHP (1-1301) | Cr. Hutang Usaha (2-1101)</span>
                            </div>
                            <span class="font-mono text-2xs font-bold px-2 py-0.5 bg-blue-500/20 rounded-lg">TERINKRON</span>
                        </div>
                    </div>

                    <div class="flex justify-end pt-3 border-t border-slate-100 dark:border-slate-800">
                        <button type="button" @click="activeDetailGrn = null" class="px-5 py-2 bg-slate-800 text-white text-xs font-bold rounded-xl cursor-pointer">
                            Tutup
                        </button>
                    </div>
                </div>
    ...
    <script>
        function grnApp(posData) {
            return {
                ...
                viewDetail(grn) {
                    this.activeDetailGrn = grn;
                },

                calculateGrnTotal(grn) {
                    if (!grn || !grn.items) return 0;
                    return grn.items.reduce((acc, i) => acc + ((parseFloat(i.quantity) || 0) * (parseFloat(i.unit_cost) || 0)), 0);
                },
