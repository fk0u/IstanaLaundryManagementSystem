<header class="flex items-center justify-between min-h-[60px] md:min-h-[64px] px-3 sm:px-6 w-full bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200/80 dark:border-slate-800/80 z-40 sticky top-0 transition-colors duration-200 shadow-2xs"
        x-data="{ 
    ...
    @php
        $pageTitle = match(true) {
            ...
            request()->is('performance*') => 'Pemantauan Kinerja & Produktivitas',
            request()->is('customers*') => 'CRM & Loyalty Pelanggan',
            request()->is('promotions*') => 'Manajemen Promosi & Kupon',
            request()->is('services*') => 'Master Layanan Cucian',
            request()->is('users*') => 'Manajemen Staf & Hak Akses',
    ...
    <div class="flex items-center gap-3 min-w-0 flex-1">
        ...
        <!-- Page Title Only (Clean Topbar) -->
        <div class="min-w-0 flex-1">
            <h1 class="text-base sm:text-lg lg:text-xl font-black font-display text-slate-900 dark:text-white tracking-tight truncate">
                {{ $pageTitle }}
            </h1>
        </div>
    </div>
    ...
    <div class="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
        ...
        <!-- 1. Quick Action Button Dropdown -->
        <div class="relative" @click.away="showQuickAction = false">
            <button @click="showQuickAction = !showQuickAction; showNotifications = false; showSupportMenu = false; showProfileMenu = false" 
                    class="md3-btn-tonal !min-h-[38px] !px-3 !rounded-full text-xs font-bold flex items-center gap-1.5 cursor-pointer shadow-2xs">
        ...
        <div class="relative" @click.away="showSupportMenu = false">
            ...
            <div x-show="showSupportMenu" 
                 ...
                 x-transition:leave-start="transform opacity-100 scale-100"
                 x-transition:leave-end="transform opacity-0 scale-95"
                 class="absolute right-0 mt-2.5 w-72 sm:w-80 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-expressive shadow-md3-3 z-50 p-4 space-y-3"
                 x-cloak>
        ...
        <div class="relative" @click.away="showNotifications = false">
            ...
            <div x-show="showNotifications" 
                 ...
                 x-transition:leave-start="transform opacity-100 scale-100"
                 x-transition:leave-end="transform opacity-0 scale-95"
                 class="absolute right-0 mt-2.5 w-80 sm:w-88 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-expressive shadow-md3-3 z-50 p-4 space-y-3"
                 x-cloak>
                    ...
                    @forelse($recentOrders as $notifOrder)
                        <a href="/production" class="flex items-start gap-3 p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
                            <span class="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center shrink-0">
                                <img alt="Istana Laundry Logo" class="w-5 h-5 object-contain" src="{{ asset('images/logo.webp') }}"/>
                            </span>
